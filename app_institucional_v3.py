...
def evidence_present(text: str, pistas: list[str]) -> bool:
    """
    Detección flexible:
    alcanza con encontrar al menos una pista en el texto.
    """
    low = text.lower()
    for kw in pistas:
        if kw.lower() in low:
...
def extract_snippets(text: str, pistas: list[str], max_snippets: int = 2) -> list[str]:
    """
    Devuelve pequeños extractos orientativos para mostrar evidencia sugerida.
    """
    low = text.lower()
    snippets = []
...
st.title(APP_TITLE)
st.caption("Cargá un proyecto (PDF/DOCX). La detección automática de evidencia es orientativa y el evaluador puede asignar puntaje manualmente.")
