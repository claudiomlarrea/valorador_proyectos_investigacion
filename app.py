...
def make_excel(scores, porcentaje, resultado, nombre_archivo):
    rows = []
...
def make_word(scores, porcentaje, resultado, nombre_archivo):
    if Document is None:
        return b""
...
scores = {}
total_max = sum(CRITERIA.values())
cols = st.columns(2)

i = 0
for criterio, peso in CRITERIA.items():
    with cols[i % 2]:
